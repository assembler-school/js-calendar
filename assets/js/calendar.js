  ...
  for (let i = 0; i < lastDay(); i++) {
    ...
    if (monthEvents != null) {
      monthEvents.forEach((event) => {
        ...
        if ( eventInitialDate >= dayUnix && eventInitialDate < tomorrowUnix ) dayEvents.push(event);
      ...
      dayEvents.forEach((event, index) => {
        ...
        if (index >= 3) return eventsHTML += `<div class="day__event-container"><div data-event class="more-events">See more events...</div></div>`;
        ...
        eventsHTML += `
          <div data-event="${event.currentIdEvent}" class="day__event-container"><span class="day__event-title ${typeEventClasses}">${event.titleEvent}</span></div>
        `;
      })
    }
    ...
    // Print day calendar
    daysMonth.innerHTML += `
        <div class="day ${currentDayClasses}">
